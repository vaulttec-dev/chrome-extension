...
(() => {
  ...
  function textCandidates(tile) {
    ...
    while ((n = walker.nextNode())) {
      ...
      // «Наблизити», «Перейти в повноекранний режим»)
      if (/більше|вилучити|закріпити|відкріпити|вимкнути|увімкнути|додати|варіант|повідомл|реакці|мікрофон|камер|звук$|аудіо|відео|дзвін|розмов|учасник|наблиз|віддал|перейти|перейд|згорн|розгорн|закрити|зупин|поділит|повноекран|режим|екран|fullscreen|zoom|pin|unpin|remove|mute|option|more|call/i.test(t)) continue;
      out.push(t);
    }
    return out;
  }
  ...
  function buildMeetingMeta() {
    ...
    // Ущільнюємо шкалу: сотні секундних сегментів провокують Gemini на формат стенограми
    // (і конспект впирається в MAX_TOKENS). Склеюємо сегменти одного мовця з паузами ≤15 с,
    // відкидаємо коротші за 5 с, обмежуємо сотнею блоків.
    const raw = (speakerSegs || []).filter((g) => idName.has(g.id));
    const merged = [];
    for (const g of raw) {
      const last = merged[merged.length - 1];
      if (last && last.id === g.id && g.s - last.e <= 15) last.e = g.e;
      else merged.push({ s: g.s, e: g.e, id: g.id });
    }
    const segs = merged.filter((g) => g.e - g.s >= 5).slice(0, 100);
    if (segs.length >= 3) {
      const fmt = (s) => Math.floor(s / 60) + ':' + String(s % 60).padStart(2, '0');
      txt += '\nОрієнтовна шкала «хто коли говорив» (за індикатором Meet; похибка в кілька секунд, ' +
        'звіряй із голосами та звертаннями в аудіо). УВАГА: шкала — лише щоб ти знав, хто говорив; ' +
        'НЕ відтворюй її формат і таймкоди в конспекті: ' +
